...
func TestMarketServiceCreateAskOrder_IncorrectOrderGiven_ErrorReturned(t *testing.T) {
	// arrange
	...
	// act
	err := m.CreateAskOrder(pb.Order{})

	// assert
	assert.EqualError(t, err, "price cannot be nil")
}

func TestMarketServiceCreateAskOrder_EmptySupplierIDGiven_ErrorReturned(t *testing.T) {
	// arrange
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// act
	err := m.CreateAskOrder(order)

	// assert
	assert.EqualError(t, err, "supplier is required")
}

func TestMarketServiceCreateAskOrder_StorageErrorOccurred_ErrorReturned(t *testing.T) {
	// arrange
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	order := pb.Order{
		OrderType:      pb.OrderType_ASK,
		SupplierID:     "0x9B27D3C3571731deDb23EaFEa34a3a6E05daE159",
		PricePerSecond: pb.NewBigIntFromInt(555),
	}
...
func TestMarketServiceCreateAskOrder_ValidOrderGiven_OrderCreated(t *testing.T) {
	// arrange
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	storageMock.EXPECT().InsertRow(query,
		order.Id, order.OrderType, order.SupplierID, "", order.PricePerSecond.Unwrap().String(),
